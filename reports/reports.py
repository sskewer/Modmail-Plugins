...
class Report(BaseCog):
    ...
    async def reportchannel(self, ctx, channel: discord.TextChannel):
        ...
        embed.add_field(
            name="Configurazione Canale",
            value=f"Canale delle segnalazioni correttamente impostato su {channel.mention}",
            inline=False,
        )
    ...
    async def reportmention(self, ctx, *, mention: str):
        ...
        embed = discord.Embed(color=discord.Color.blue(), timestamp=datetime.utcnow())
        embed.add_field(
            name="Configurazioni Menzioni",
            value=f"Menzioni delle segnalazioni correttamente impostato su {mention}",
            inline=False,
        )
    ...
    async def report(self, ctx, user: discord.Member, *, reason: str):
        ...
        embed2 = discord.Embed(
            title="**Riepilogo Segnalazione**", color=discord.Color.red(), timestamp=datetime.utcnow()
        )

...
class TicketManagement(commands.Cog):
    ...
    async def open(self, ctx: Context, user: discord.Member):
        ...
        # Embed
        embed = discord.Embed(title="**Richiesta Accettata**", description=f"Hey <@{user.id}>, la tua richiesta è stata accettata e per questo abbiamo aperto un ticket. Un membero del team Vindertech ti risponderà il prima possibile.", color=discord.Color.green(), timestamp=datetime.datetime.utcnow())
